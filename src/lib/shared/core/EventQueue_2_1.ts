...
export interface EmitState {
  accessibleChildrenExecutorStates?: any;
...
let _glob_: T_glob_ = {
  ...
  globExecState: new Map<string, any[]>(),
  stores: {},
  openStores: {}
};
...
export class EventQueueAlt {
  ...
  public openStore = {
    ...
    remove(name: string, data: any | any[]) {
      if (!_glob_.openStores[this.instanceId][name])
        _glob_.openStores[this.instanceId][name] = {};
      const _in = { ..._glob_.openStores[this.instanceId][name] };
      let _out = _in;
